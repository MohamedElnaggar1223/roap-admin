...
import { editSpokenLanguage } from "@/lib/actions/spoken-languages.actions"

const updateSpokenLanguageSchema = z.object({
    name: z.string().min(2, {
...
export default function EditSpokenLanguage({ spokenLanguage, spokenLanguageId }: Props) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)

    const form = useForm<z.infer<typeof updateSpokenLanguageSchema>>({
        resolver: zodResolver(updateSpokenLanguageSchema),
        defaultValues: {
            name: spokenLanguage?.name || '',
        },
    })

    async function onSubmit(values: z.infer<typeof updateSpokenLanguageSchema>) {
        try {
            setLoading(true)

            const result = await editSpokenLanguage({
                name: values.name,
                id: parseInt(spokenLanguageId)
            })
    ...
    return (
        <div className="flex flex-col w-full items-center justify-start h-full gap-6">
            <div className="flex max-w-7xl items-center justify-between gap-2 w-full">
                <h1 className="text-3xl font-bold">Edit Spoken Language</h1>
            </div>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-full max-w-7xl">
                    ...
                    <div className="space-y-4 max-w-7xl w-full space-x-2">
                        <Button
                            disabled={loading || (form.getValues('name') === spokenLanguage?.name)}
                            type='submit'
                            variant="outline"
